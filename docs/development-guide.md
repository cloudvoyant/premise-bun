...
```
packages/premise-bun/               # premise-bun package
packages/premise-bun/src/index.ts   # Public API -- re-exports from lib modules
packages/premise-bun/src/lib.ts     # Library implementation
packages/premise-bun/tests/         # bun test suite
packages/premise-bun/dist/          # Compiled output (generated by `mise run build`)
packages/premise-bun/package.json   # Package metadata and npm scripts
packages/premise-bun/tsconfig.json  # TypeScript compiler config
...
1. **Add a feature**: write code in `packages/premise-bun/src/`, add tests in `packages/premise-bun/tests/`
